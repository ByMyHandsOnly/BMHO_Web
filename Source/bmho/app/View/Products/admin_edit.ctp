...
<div class="products form">
    ...
    <fieldset>
        <legend><?php echo __('Edit Item'); ?> <small><?php echo __('[Admin]'); ?></small> <?php echo $this->Html->link('Manage Images', array('action' => 'view', $this->request->data['Product']['id']), array('class' => 'btn btn-small pull-right')); ?></legend>
        <hr />
		<?php
...
//			'helpBlock' => 'help block here.'
		...
		echo $this->Form->input('shop_id', array(
			...
			'style' => 'width: auto',
			'helpInline' => '<span class="label label-important">' . __('Required') . '</span>&nbsp;',
			'helpBlock' => __('Select the Owning Shop name here.')
		));
		...
		echo $this->Form->input('product_category_id', array(
			...
			'class' => 'input-xlarge span2',
			'helpInline' => '<span class="label label-important">' . __('Required') . '</span>&nbsp;',
			'helpBlock' => __('Select an Owner Category name here.')
		));
