...
<div class="products form">
    ...
    <fieldset>
        <legend><?php echo __('Sell Item - Premium'); ?> <small><?php echo __('[Seller]'); ?></small></legend>
        <hr />
		<?php
		...
		echo $this->Form->input('ProductImage.0.image', array(
			...
			'class' => 'input-xlarge',
			'helpInline' => '<span class="label label-important">' . __('Required') . '</span>&nbsp;',
			'helpBlock' => __('Browse and select your product\'s image.') . ' ' . __('Max file size is 5mb.') . '<br /><br /><div id="panelImages"></div><a id="btn btn-small addImage" class="btn btn-small btn-primary" icon="plus white">' . __('Add Images') . '</a>',
			'type' => 'file',
			'style' => 'width: auto'
		));
