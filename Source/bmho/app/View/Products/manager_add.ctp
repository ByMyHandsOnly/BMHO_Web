...
<div class="products form">
    ...
    <fieldset>
		...
		echo $this->Form->input('ProductImage.0.image', array(
			...
			'class' => 'input-xlarge',
			'helpInline' => '<span class="label label-important">' . __('Required') . '</span>&nbsp;',
			'helpBlock' => __('Browse and select Item image.') . ' ' . __('Max file size is 5mb.') . '<br /><br /><div id="panelImages"></div><a id="addImage" class="btn btn-warning" icon="plus white">' . __('Add Image') . '</a>',
			'type' => 'file',
			'style' => 'width: auto'
		));
