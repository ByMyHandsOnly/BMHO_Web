...
<?php if ($wannaUseSession): ?>
				$this->Session->setFlash(__('The <?php echo strtolower($singularHumanName); ?> could not be saved. Please, try again.'), 'flash_fail');
<?php endif; ?>
			}
		}
<?php
...
<?php else: ?>
		$this->flash(__('<?php echo ucfirst(strtolower($singularHumanName)); ?> was not deleted'), array('action' => 'index'), 'flash_fail');
<?php endif; ?>
		$this->redirect(array('action' => 'index'));
	}
